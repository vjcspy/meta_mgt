...
namespace Chiaki\CatalogGraphql\Model\Resolver\Category;
...
/**
 * Resolve app background image to a fully qualified URL
 */
class AppBackgroundImage implements ResolverInterface
{
    /** @var DirectoryList  */
    ...
    public function resolve(
        ...
        $imagePath = $category->getData('app_background_image');
        if (empty($imagePath)) {
            return null;
        }
        ...
        if (!$this->fileInfo->isExist($filenameWithMedia)) {
            throw new GraphQlInputException(__('Category mobile banner image not found.'));
        }
